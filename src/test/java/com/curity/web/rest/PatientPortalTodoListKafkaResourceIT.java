...
import com.curity.IntegrationTest;
import com.curity.config.EmbeddedKafka;
import com.curity.config.KafkaSseConsumer;
import com.curity.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class PatientPortalTodoListKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/patient-portal-todo-list-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/patient-portal-todo-list-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/patient-portal-todo-list-kafka/unregister"));
                return;
            }
        }
